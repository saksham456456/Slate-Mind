...
async function groqChat(messages, timeoutMs = 25000, maxTokens = 800) {
...
async function generateLesson(topic) {
  ...
  const raw    = await groqChat(messages, 25000, 800);
...
async function generateAnswer(question, history) {
  ...
  const raw    = await groqChat(messages, 25000, 250);
  const parsed = parseBlockJSON(raw);
  return {
    parsed,
